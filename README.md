...
RailsはOpenStreetMap Nominatimの逆ジオコーディングとOverpass APIを使い、道路・地域名と近隣の名称付きスポットを取得してOpenAIへ渡します。公開サービスを使うため、アプリ内で各サービスを毎秒1回以下に制限し、OpenStreetMapの帰属を画面に表示しています。小規模なMVP用途を超える場合は、専用インスタンスまたは商用の地図サービスへ切り替えてください。
