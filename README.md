...
RailsはOpenStreetMap Nominatimの逆ジオコーディングを使い、座標から道路・地域名を取得してOpenAIへ渡します。公開Nominatimの利用条件に合わせ、アプリ内で毎秒1回以下に制限し、OpenStreetMapの帰属を画面に表示しています。小規模なMVP用途を超える場合は、専用インスタンスまたは商用の地図サービスへ切り替えてください。
