...
`drive_guide_controller.js` は現在地を取得した後、Google Maps JavaScript APIを遅延読み込みして地図の中心と「現在地」マーカーを更新する。開発環境では九州工業大学飯塚キャンパス（緯度33.65409392、経度130.67159183）を初期位置として表示する。開発用シミュレーションでも同じ更新処理を使う。さらに開発環境だけは、利用者が地図を移動してから「地図中心でシミュレーション」を実行できる。この操作は地図中心を一度だけ通常のガイドAPIへ渡すだけで、GPS位置の変更や座標の保存は行わない。`GOOGLE_MAPS_API_KEY` はブラウザで利用するため公開されるが、OpenAIキーとは別の専用キーとし、HTTPリファラーとMaps JavaScript APIだけに制限する。未設定または読み込み失敗時も、ガイド取得・読み上げは継続する。
